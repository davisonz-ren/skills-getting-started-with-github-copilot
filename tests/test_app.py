...
def test_signup_and_unregister():
    ...
    client.post(f"/activities/{activity}/unregister", json={"name": test_email})
    ...
    # Unregister
    response = client.post(f"/activities/{activity}/unregister", json={"name": test_email})
    assert response.status_code == 200
...
def test_unregister_not_found():
    ...
    response = client.post(f"/activities/{activity}/unregister", json={"name": email})
